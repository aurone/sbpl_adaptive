...
bool PViz::computeFKforVisualizationWithKDL(const std::vector<double> &jnt0_pos, std::vector<double> &jnt1_pos, std::vector<double> &base_pos, double torso_pos, std::vector<geometry_msgs::PoseStamped> &poses, std::vector<int> &mesh_ids)
{
...
*/
  ...
  poses.reserve(robot_meshes_.size());
  mesh_ids.reserve(robot_meshes_.size());

  for(int i=0; i <  int(robot_meshes_.size()); i++)
  {
    // base, torso
    if(i < 2){
      if(!computeFKwithKDL(jnt0_pos, base_pos, torso_pos, RIGHT, i+1, tmp_pose.pose))
        return false;
      poses.push_back(tmp_pose);
      mesh_ids.push_back(i);
    }
    // right arm thru the right finger
    else if(i < 14)
    {
      if(jnt0_pos.size() == 0) continue;
      if(!computeFKwithKDL(jnt0_pos, base_pos, torso_pos, RIGHT, i+1, tmp_pose.pose))
        return false;
      poses.push_back(tmp_pose);
      mesh_ids.push_back(i);
    ...
    {
      if(jnt0_pos.size() == 0) continue;
      if(!computeFKwithKDL(jnt0_pos, base_pos, torso_pos, RIGHT, -1*(i-1), tmp_pose.pose))
        return false;
      poses.push_back(tmp_pose);
      mesh_ids.push_back(i);
    }
    // left arm thru the right finger
     ...
     {
      if(jnt1_pos.size() == 0) continue;
      if(!computeFKwithKDL(jnt1_pos, base_pos, torso_pos, LEFT, i-13, tmp_pose.pose))
        return false;
      poses.push_back(tmp_pose);
      mesh_ids.push_back(i);
    }
    ...
    {
      if(jnt1_pos.size() == 0) continue;
      if(!computeFKwithKDL(jnt1_pos, base_pos, torso_pos, LEFT, -1*(i-15), tmp_pose.pose))
        return false;
      poses.push_back(tmp_pose);
      mesh_ids.push_back(i);
    }
...
void PViz::visualizeRobotMeshes(double hue, std::string ns, int &start_id, std::vector<geometry_msgs::PoseStamped> &poses, std::vector<int> mesh_ids)
{
  double r,g,b;
  marker_array_.markers.clear();
  ...
  for(int i = 0; i < (int)marker_array_.markers.size(); ++i)
  {
    ...
    //ROS_INFO("i=%d",i);
    marker_array_.markers[i].mesh_resource = robot_meshes_[mesh_ids[i]];
  }
  start_id += marker_array_.markers.size();
...
void PViz::visualizeRobot(std::vector<double> &jnt0_pos, std::vector<double> &jnt1_pos, std::vector<double> &base_pos, double torso_pos, double hue, std::string ns, int &start_id)
{
  ...
  if(!computeFKforVisualizationWithKDL(jnt0_pos, jnt1_pos, base_pos, torso_pos, poses, mesh_ids))

    ROS_WARN("Unable to compute forward kinematics.");
  else
    visualizeRobotMeshes(hue, ns, start_id, poses, mesh_ids);
}

void PViz::visualizeRobot(std::vector<double> &jnt0_pos, std::vector<double> &jnt1_pos, BodyPose &body_pos, double hue, std::string ns){
  int start_id = 0;
...
bool PViz::visualizeTrajectoryFromFile(std::string filename)
{
  ...
  for(size_t i = 0; i < traj.size(); ++i)
  {
    ...
    visualizeRobot(jnt0_pos,jnt1_pos,base_pos,torso,(30*i)%300,"full_body_waypoint_"+boost::lexical_cast<std::string>(i));
    ros::spinOnce();
    sleep(1.0);
  }
...
void PViz::visualizeArmConfigurations(const std::vector<std::vector<double> > &traj, int arm, int throttle, std::string name, std::string frame_id){

  std::string tmp = reference_frame_;
  reference_frame_ = frame_id;
  ...
  getPR2State(body_pos, rangles, langles);
  ...
  for (int i = 0; i < length; ++i) {
    ...
    ma1 = getRobotMarkerMsg(rangles, langles, body_pos, (i / throttle) * color_inc, name, (i+1)*30);
    ma.markers.insert(ma.markers.end(), ma1.markers.begin(), ma1.markers.end()); 
  }
  ROS_INFO("[pviz] Visualizing arm path with %d waypoints. (throttle = %d)", length, throttle);
  marker_array_publisher_.publish(ma);
  reference_frame_ = tmp;
}
    
void PViz::visualizeArmConfiguration(const std::vector<double> &traj, int arm, double hue, std::string name, std::string frame_id){
  ...
  ma1 = getRobotMarkerMsg(rangles, langles, body_pos, hue, name, 0);
  ROS_INFO("[pviz] Visualizing arm config (%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f)!", traj[0],traj[1],traj[2],traj[3],traj[4],traj[5],traj[6]);
  marker_array_publisher_.publish(ma1);
  reference_frame_ = tmp;
}

void PViz::getPR2State(BodyPose &body_pos, std::vector<double> &rangles, std::vector<double> &langles)
{
	rangles.resize(7);
	langles.resize(7);
	...
	for(int i = 0; i < (int)state->name.size(); i++){
		if(state->name[i].compare("r_shoulder_pan_joint")==0) rangles[0] = state->position[i];
		if(state->name[i].compare("r_shoulder_lift_joint")==0) rangles[1] = state->position[i];
		if(state->name[i].compare("r_upper_arm_roll_joint")==0) rangles[2] = state->position[i];
		if(state->name[i].compare("r_elbow_flex_joint")==0) rangles[3] = state->position[i];
		if(state->name[i].compare("r_forearm_roll_joint")==0) rangles[4] = state->position[i];
		if(state->name[i].compare("r_wrist_flex_joint")==0) rangles[5] = state->position[i];
		if(state->name[i].compare("r_wrist_roll_joint")==0) rangles[6] = state->position[i];
		
		if(state->name[i].compare("l_shoulder_pan_joint")==0) langles[0] = state->position[i];
		if(state->name[i].compare("l_shoulder_lift_joint")==0) langles[1] = state->position[i];
		if(state->name[i].compare("l_upper_arm_roll_joint")==0) langles[2] = state->position[i];
		if(state->name[i].compare("l_elbow_flex_joint")==0) langles[3] = state->position[i];
		if(state->name[i].compare("l_forearm_roll_joint")==0) langles[4] = state->position[i];
		if(state->name[i].compare("l_wrist_flex_joint")==0) langles[5] = state->position[i];
		if(state->name[i].compare("l_wrist_roll_joint")==0) langles[6] = state->position[i];
	}
	body_pos.z = state->position[12];
	...
	try {
		...
		body_pos.theta = 2 * acos(base_map_transform_.getRotation().getW());
	}
	catch (tf::TransformException& ex) {
		ROS_ERROR("Is there a map? The map-robot transform failed. (%s)", ex.what());
	}*/
	
	/*ROS_WARN("Got the following robot state:");
	ROS_WARN("\t LArm (%.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f)", langles[0], langles[1], langles[2], langles[3], langles[4], langles[5], langles[6]);
	ROS_WARN("\t RArm (%.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f)", rangles[0], rangles[1], rangles[2], rangles[3], rangles[4], rangles[5], rangles[6]);
	ROS_WARN("\t Base (%.3f, %.3f, %.3f, %.3f)", body_pos.x, body_pos.y, body_pos.theta, body_pos.z);*/
}
