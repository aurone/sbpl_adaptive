...
{
    ros::init(argc, argv, "benchmark_sbpl_adaptive_collision_checking");
    ...
    // set up the grid
    OccupancyGrid3D* grid_ = new OccupancyGrid3D(1.7, 1.9, 2.0, 0.01, -0.6,
            -1.25, -0.05, 0.40);
    ...
    // get map
    moveit_msgs::CollisionMapConstPtr map = ros::topic::waitForMessage<
            moveit_msgs::CollisionMap>("collision_map_occ");
    ...
    ROS_INFO("checks per second: %0.8fsec",
            1.0 / (total_time / double(num_checks)));

    ROS_INFO("[sanity check] # calls: %d   # calls not in collision: %d",
            cspace_->num_collision_checks_,
            cspace_->num_false_collision_checks_);
